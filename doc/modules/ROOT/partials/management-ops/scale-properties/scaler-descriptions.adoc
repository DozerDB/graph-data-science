...
Scales all property values using the https://en.wikipedia.org/wiki/Norm_(mathematics)#Taxicab_norm_or_Manhattan_norm[L1 Norm (Wikipedia)], also known as Manhattan norm.
...
The scaler is still applicable when negative values are present, but will not produce a probability distribution. In either case, the sum of scaled absolute values will equal 1.


[[algorithms-scale-properties-l2norm]]
=== L2 Norm

Scales all property values using the https://en.wikipedia.org/wiki/Norm_(mathematics)#Euclidean_norm[L2 Norm (Wikipedia)], also known as Euclidean norm.
