...
[NOTE]
====
Publishing a model via the xref:aura-graph-analytics/cypher.adoc[Cypher API for Aura Graph Analytics] will also xref:model-catalog/store.adoc[store] the model.
====
...
----
CALL gds.model.publish([ sessionName: String or Map, ] modelName: String)
YIELD
    modelName: String,
    modelType: String,
    modelInfo: Map,
...
| sessionName label:serverless[Aura Graph Analytics]    | String or Map     | n/a     | no       | The name of the xref:aura-graph-analytics/index.adoc[GDS Session] from which the models are listed. Accepts the output of the helper function `sessionInfo`.
